...
/**
 * Inject the Voqi widget <script> tag at runtime. The widget exposes a
 * ``window.VoqiReady`` queue, so order-vs-registration doesn't matter
 * — tool handlers in voqi.ts push into the queue and run whenever the
